export class EventEmitter {
    ...
    on<Data = unknown, Event = unknown>(
        ...
        this.listeners.set(
            evt,
            (this.listeners.get(evt) || new Set()).add(callback as any)
        );

        return () => this.off(event, callback);
    }

    once<Data = unknown, Event = unknown>(
        event: Event,
        callback: (data: Data) => void
    ) {
    ...
    off<Data = unknown, Event = unknown>(
        event: Event,
        callback?: (data: Data) => void
    ) {
    ...
    dispatch<Event = unknown, Data = unknown>(event: Event, data: Data) {
        const evt = event as unknown as string;
        const listeners: EventCallback[] = Array.from(
            this.listeners.get(evt) || new Set()
